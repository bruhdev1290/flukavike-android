...
- Stale `EncryptedSharedPreferences` session cookie caused infinite loading on startup after reinstall.
...
- Current user is `/api/users/@me` — NOT `/api/auth/me` (returns 404)
...
- `Message.authorId`, `Message.content`, `Message.createdAt` all default to `""` — API omits these in certain contexts
